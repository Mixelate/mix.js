...
export function AplikoBuildEmbeds(client: Client, ...aplikoEmbeds: AplikoEmbed[]): MessageEmbed[] {
    ...
    for (const aplikoEmbed of aplikoEmbeds) {
        ...
        switch (aplikoEmbed.style) {
            ...
            case AplikoEmbedStyle.CUSTOM: {
                messageEmbed.setColor(aplikoEmbed.customStyle ? aplikoEmbed.customStyle : AplikoEmbedPrimaryColor);
                break;
            }

            default: {
                if (client.options.apprearance?.embedPrimaryColor) messageEmbed.setColor(client.options.apprearance.embedPrimaryColor);
        ...
        if (aplikoEmbed.author) {
            if (aplikoEmbed.author.name.length > AplikoEmbedMaxAuthorNameSize) ThrowError('Embed author name is too long');
        ...
        if (aplikoEmbed.title) {
            if (aplikoEmbed.title.length > aplikoEmbedMaxTitleSize) ThrowError('Embed title is too long');
        ...
        if (aplikoEmbed.description) {
            if (aplikoEmbed.description.length > AplikoEmbedMaxDescriptionSize) ThrowError('Embed description is too long');
        ...
        if (aplikoEmbed.footer) {
            if (aplikoEmbed.footer.content.length > AplikoEmbedMaxFooterContentSize) ThrowError('Embed footer is too long');
        ...
        // Set thumbnail
        if (aplikoEmbed.thumbnail) messageEmbed.thumbnail = isMessageEmbedThumbnail(aplikoEmbed.thumbnail) ? aplikoEmbed.thumbnail : { url: aplikoEmbed.thumbnail };

        // Add fields
        if (aplikoEmbed.fields && aplikoEmbed.fields.length > 0) {
            if (aplikoEmbed.fields.length > AplikoEmbedMaxFields) ThrowError('Embed has too many fields');

            for (const aplikoEmbedField of aplikoEmbed.fields) {
                if (aplikoEmbedField.title.length > AplikoEmbedMaxFieldTitleSize) ThrowError('Embed field title is too long');

                if (aplikoEmbedField.content && aplikoEmbedField.content.length > AplikoEmbedMaxFieldContentSize) ThrowError('Embged field content is too long');
...
function isMessageEmbedThumbnail(thumbnail: any): thumbnail is MessageEmbedThumbnail {
    try {
