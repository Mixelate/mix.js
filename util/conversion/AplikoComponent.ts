...
export function isAplikoInteractButton(
  aplikoComponent: AplikoComponent
): aplikoComponent is AplikoInteractButton {
...
export function isAplikoLinkButton(
  aplikoComponent: AplikoComponent
): aplikoComponent is AplikoLinkButton {
...
export function isAplikoDropdown(
  aplikoComponent: AplikoComponent
): aplikoComponent is AplikoDropdown {
  return "options" in aplikoComponent;
}

export type AplikoComponent =
  | AplikoInteractButton
  | AplikoLinkButton
  | AplikoDropdown;

export function AplikoBuildComponent(
  aplikoComponent: AplikoComponent
  ...
  if (isAplikoInteractButton(aplikoComponent)) {
    ...
    if (aplikoComponent.emoji) messageButton.setEmoji(aplikoComponent.emoji);
  ...
  if (isAplikoDropdown(aplikoComponent)) {
    const messageSelect = new MessageSelectMenu()
      ...
      .setOptions(
        aplikoComponent.options.map((option) => {
          return {
            label: option.title,
    ...
    if (aplikoComponent.min) messageSelect.setMinValues(aplikoComponent.min);

    if (aplikoComponent.max) messageSelect.setMaxValues(aplikoComponent.max);
...
export function AplikoBuildComponentRows(
  ...componentRows: AplikoComponent[][]
): MessageActionRow[] {
  ...
  componentRows.forEach((row, rowIdx) => {
    ...
    row.forEach((component, componentIdx) => {
      const compiledComponent =
        AplikoBuildComponent(component) ??
        ThrowError(`Invalid component data at [${rowIdx}][${componentIdx}]`);

      compiledRow.addComponents(
        compiledComponent as MessageActionRowComponentResolvable
      );
    });
