...
export function AplikoBuildEmbeds(...aplikoEmbeds: AplikoEmbed[]): MessageEmbed[] {
    ...
    for (const aplikoEmbed of aplikoEmbeds) {
        ...
        switch (aplikoEmbed.style) {
            ...
            default: {
                if (APLIKO_OPTIONS?.apprearance?.embedPrimaryColor)
                    ...
                    messageEmbed.setColor(AplikoEmbedPrimaryColor)
            }
        }
        ...
        if (aplikoEmbed.thumbnail)
            messageEmbed.thumbnail =
                isMessageEmbedThumbnail(aplikoEmbed.thumbnail)
        ...
        if (aplikoEmbed.fields && aplikoEmbed.fields.length > 0) {
            ...
            for (const aplikoEmbedField of aplikoEmbed.fields) {
                ...
                if (AplikoEmbedTwoColumnRows && aplikoEmbedField.inline
                    && (messageEmbed.fields.length % 4 === 0 || messageEmbed.fields.length === 1))
                    messageEmbed.addField(
                        '\u200b', // Invis space
                        '\u200b', // Invis space
                        true
