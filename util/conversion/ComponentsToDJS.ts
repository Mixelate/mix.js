...
export function ComponentsToDJS(
  ...
  for (const actionRow of apiActionRows) {
    ...
    for (const component of actionRow.components) {
      if (IsApiButton(component)) {
        ...
        if (IsApiLinkButton(component)) djsButton.setURL(component.url);

        if (component.emoji) djsButton.setEmoji(component.emoji);
      ...
      if (IsApiDropdown(component)) {
        ...
        djsDropdown.setPlaceholder(
          component.placeholder ? component.placeholder : "Select an option"
        );

        if (IsApiMultiSelectDropdown(component)) {
          djsDropdown.setMinValues(
            component.min_values ? component.min_values : 1
          );
          djsDropdown.setMaxValues(
            component.max_values == "MAX"
