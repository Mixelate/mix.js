...
export class RepliableInteraction extends BaseInteraction {
    ...
    public async deferReply(ephemeral?: boolean) {
        if (this.deferredEdit)
            throw new AplikoError(
                `defer() called before deferReply()`,
                [
                    `Use deferReply if you want to create a new message.`,
                    `Use defer() if you want to edit the original message`
                ]
            )

        if (this.replied)
            throw new AplikoError(
                `reply() called before deferReply()`,
                [`If you want to defer the reply, try calling deferReply() before reply()`]
            )

        if (this.deferredReply)
            return; // Silent catch deferring twice

        await this.getBot().rest.post(Routes.interactionCallback(this.getId(), this.getToken()), {
            body: {
    ...
    public async reply(message: AplikoMessage, ephemeral?: boolean) {
        ...
        if (this.deferredReply)
            throw new AplikoError(
                `deferReply() was called before reply().`,
                [
                    `Try calling editReply() instead of reply()`
                ]
            );

        if (this.replied)
            throw new AplikoError(
                `reply() was called twice.`,
                [`Try calling editReply() instead.`]
            )
            ...
            body: {
                ...
                data: {
                    ...
                    components: message.components
                        ? ComponentsToDJS(...message.components)
                        : [],
                },
            },
        });
   ...
   */
    public async editReply(message: AplikoMessage) {
        if (this.deferredEdit)
            throw new AplikoError(
                `defer() was called before editReply().`,
                [`Try calling deferReply() instead of defer()`]
            );

        if (!this.replied && !this.deferredReply)
            throw new AplikoError(
                `editReply() was called before deferReply() or reply()`,
                [`Try calling reply() instead of editReply()`]
            )

        await this.getWebhook().editMessage("@original", {
            content: message.content ? message.content : undefined,
            embeds: message.embeds ? AplikoBuildEmbeds(...message.embeds) : [],
            components: message.components
                ? ComponentsToDJS(...message.components)
    ...
    public async defer() {
        if (this.deferredReply)
            throw new AplikoError(
                `deferReply() called before defer()`,
                [
                    `Use deferReply if you want to create a new message.`,
                    `Use defer() if you want to edit the original message`
                ]
                    ...
                    embeds: [
                        {
                            ...
                            description: error instanceof AplikoError
                                ? 'A framework error has occurred. Contact an administrator'
