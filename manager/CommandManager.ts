...
export class CommandManager {
    ...
    private async onInteractionCreate(interaction: Interaction) {
        ...
        await this._commandMap.get(interaction.commandName)
    ...
    public async createCommandsGlobal() {
        ...
        guilds.forEach(guild => {
            ...
            if (!resolvedGuild)
                return console.log(`Skipped command registration on guild: ${guild.id} (Failed to resolve guild)`)
    ...
    public async createCommandsInGuild(guild: Guild) {
        ...
        guildCommands
            .filter(guildCommand => guildCommand.applicationId === this._bot.client.application!.id)
            .forEach(guildCommand => {
                ...
                commandsToRegister = commandsToRegister
                    .filter(command => command !== matchingCommand)
            })
