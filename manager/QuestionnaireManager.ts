...
import { IsQuestionnaireSelectQuestion, Questionnaire, QuestionnaireContext } from "../struct/apliko/questionnaire";
import { AplikoBuildComponentRows, AplikoBuildEmbeds, AplikoDropdown, AplikoDropdownOption, AplikoEmbed, AplikoEmbedField, AplikoEmbedStyle, AplikoInteractButton, RepliableInteraction, ThrowError } from "../util";
...
export class QuestionnaireManager {
    ...
    public async start(interaction: RepliableInteraction, questionnaire: Questionnaire): Promise<QuestionnaireContext> {
        const context = {
            questionnaire,
            revising: false,
            answers: []
    ...
    public async revise(interaction: RepliableInteraction, context: QuestionnaireContext): Promise<QuestionnaireContext> {
        ...
        const value = (await Promise.race([
            ...
            this.bot.collector.collectButton({
                guildMemberResolvable: interaction.user.id,
                messageResolvable: message.id,
    ...
    public async askQuestion(interaction: RepliableInteraction, context: QuestionnaireContext, questionIndex: number, error?: string) {
    ...
    public exportTyped<T>(context: QuestionnaireContext): any {
        ...
        context.questionnaire.questions.forEach((question, index) => {
            if (!question.key)
                ThrowError('Question incorrectly configured: missing key property. Report to an administrator.')
