...
export function RegisterCallbacks<T extends CallbackHandlerConstructor>(
  Class: T
) {
  return class extends Class {
    constructor(...args: any[]) {
      ...
      if (this.loadCallbacks && loadCallbacks)
        for (const callback of loadCallbacks) this.loadCallbacks.push(callback);

      if (this.stopCallbacks && stopCallbacks)
        for (const callback of stopCallbacks) this.stopCallbacks.push(callback);
