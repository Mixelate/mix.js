...
export function DefineCommand<T extends { new (...args: any[]): Command }>(
  Base: T
) {
...
export const CommandExecutor = (
  ...
  if (
    ...
    return ThrowError(
      `APLIKO_COMMAND_PARAMS_MISSING_INTERACTION: ${propertyKey}`
    );
  ...
  if (paramTypes.length > 1) {
    ...
    optionTypes.forEach((paramType, idx) => {
      ...
      if (paramTypeString.includes("TextChannel")) {
        options.push(<AplikoCommandChannelOption>{
          type: "channel",
      ...
      if (paramTypeString.includes("GuildMember")) {
        options.push(<AplikoCommandUserOption>{
          type: "user",
          name: optionNames[idx],
          description: "Member",
          required: true,
      ...
      if (paramTypeString.includes("Role")) {
        options.push(<AplikoCommandRoleOption>{
          type: "role",
          name: optionNames[idx],
          description: "Role",
          required: true,
      ...
      if (paramTypeString.includes("string")) {
        options.push(<AplikoCommandStringOption>{
          type: "string",
          choices: undefined,
          name: optionNames[idx],
          description: "Text",
      ...
      if (paramTypeString.includes("number")) {
        options.push(<AplikoCommandIntegerOption>{
          type: "integer",
          name: optionNames[idx],
          description: "Number",
          required: true,
