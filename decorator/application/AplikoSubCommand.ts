...
export const SubCommandExecutor =
  ...
  (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
    ...
    if (
      paramTypes.length == 0 ||
      !paramTypes[0].toString().includes(" CommandInteraction ")
    )
      return ThrowError(
        `APLIKO_SUB_COMMAND_PARAMS_MISSING_INTERACTION: ${propertyKey}`
      );
    ...
    if (paramTypes.length > 1) {
      ...
      optionTypes.forEach((paramType, idx) => {
        ...
        if (paramTypeString.includes("TextChannel")) {
          subCommand.options!.push(<AplikoCommandChannelOption>{
            type: "channel",
        ...
        if (paramTypeString.includes("GuildMember")) {
          subCommand.options!.push(<AplikoCommandUserOption>{
            type: "user",
            name: optionNames[idx],
            description: "Member",
            required: true,
        ...
        if (paramTypeString.includes("Role")) {
          subCommand.options!.push(<AplikoCommandRoleOption>{
            type: "role",
            name: optionNames[idx],
            description: "Role",
            required: true,
        ...
        if (paramTypeString.includes("string")) {
          subCommand.options!.push(<AplikoCommandStringOption>{
            type: "string",
            choices: undefined,
            name: optionNames[idx],
            description: "Text",
        ...
        if (paramTypeString.includes("number")) {
          subCommand.options!.push(<AplikoCommandIntegerOption>{
            type: "integer",
            name: optionNames[idx],
            description: "Number",
            required: true,
