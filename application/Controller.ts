...
export class Controller implements CallbackHandler {
  ...
  public async load() {
    ...
    this.loadCallbacks.forEach(
      async (loadCallback) => await loadCallback.apply(this)
    );
  }
  ...
  public async stop() {
    ...
    this.stopCallbacks.forEach(
      async (stopCallback) => await stopCallback.apply(this)
    );
  }
  ...
  public async onMessageCreate(message: Message) {
    this.messageCallbacks.forEach((messageCallback) =>
      messageCallback.apply(this, [message])
    );
  }

  public async getChannelInstance(): Promise<TextChannel> {
    const apiChannel =
      (await this._bot.client.channels.fetch(this._channelId)) ??
      ThrowError("APLIKO_CHANNEL_CONTROLLER_GET_INST_FALSY_API");

    const resolvedChannel =
      this._bot.client.channels.resolve(apiChannel) ??
