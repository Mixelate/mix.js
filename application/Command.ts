...
export class Command {
  ...
  public async callExecutors(interaction: CommandInteraction) {
    try {
      if (this._subCommands.length > 0) {
        try {
          if (interaction.options.getSubcommand()) {
            const subCommand = this._subCommands.find(
              (subCommand) =>
            ...
            return subCommand.callback.apply(this, [
              interaction,
              ...this.resolveOptions(interaction),
            ]);
          }
        } catch (err) {
      ...
      return this._executor.apply(this, [
        interaction,
        ...this.resolveOptions(interaction),
      ]);
    } catch (error: any) {
  ...
  public resolveOptions(interaction: CommandInteraction): any[] {
    ...
    if (interaction.options.data[0]?.options) {
      for (const option of interaction.options.data[0].options!) {
        const object =
          this.resolveOption(interaction, option) ||
