import { SlashCommandBuilder, SlashCommandChannelOption, SlashCommandIntegerOption, SlashCommandRoleOption, SlashCommandStringOption, SlashCommandSubcommandBuilder, SlashCommandUserOption } from "@discordjs/builders";
import { ChannelType } from "discord-api-types";
import { ApplicationCommandOptionType, CategoryChannel, CommandInteraction, CommandInteractionOption, PermissionResolvable, RoleResolvable, TextChannel, UserResolvable } from "discord.js";
...
export class Command {
    ...
    public readonly _bot: AplikoBot
    public _name: string
            ...
            if (this._subCommands.length > 0) {
                try {
                    if (interaction.options.getSubcommand()) {
                        const subCommand = this._subCommands.find(subCommand =>
                            subCommand.name === interaction.options.getSubcommand())
            
                        if (!subCommand)
                            ThrowError('COMMAND_MISSING_SUB_COMMAND')
    ...
    public resolveOptions(interaction: CommandInteraction): any[] {
        ...
        if (interaction.options.data[0]?.options) {
            for (const option of interaction.options.data[0].options!) {
                const object = this.resolveOption(interaction, option) ||
                    ThrowError('COMMAND_FALSY_RESOLVED_OPTION: ' + option.name)
...
export type AplikoCommandOption =
